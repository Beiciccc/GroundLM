...
high and stable through mid layers on all four families (per-layer curves use all
cells; the collision-free supp$_{O=1}$ is in Table~\ref{tab:c1}). The green star marks
the same probe and metric at the a-priori mid layer with the context \emph{removed}
(\S\ref{sec:c1}): the raw AUROC barely moves (per-panel drop annotated; the
confidence-purged value is likewise unchanged and shuffled context behaves the same),
so the within-stratum contrast is mostly question--answer compatibility rather than
context use.}
\label{fig:c1layers}
\end{figure*}
...
these numbers, so the negative is if anything understated; on real data overlap still
dominates.
...
pooled $0.753$ falls to macro $0.647$, NLI's $0.673$ to $0.582$. The \emph{Sent-NLI} rows are the
SummaC-style sentence-level baseline; they do no better (macro $0.596$/$0.579$). Within
...
at $\mathrm{corr}{=}0.50$ with overlap stratification. (iii)~The cosine alignment is
measured on shared stimuli, so it shows alignment of a shared low-rank contrast, not a
fully unpaired transfer. (iv)~RAGTruth covers three task types and one annotation
scheme, and the negative is on $3/4$ families (Mistral's RAGTruth extraction OOM'd);
the in-domain probe is scored with response-level grouping, which does not hold out
passages shared across responses and is therefore, if anything, an optimistic ceiling.
(v)~Stronger internals detectors (e.g.\ ReDeEP \citep{sun2025redeep}) and
sampling-based methods are out of our single-A100 budget, so we show that \emph{this}
synthetic axis fails, not that internals cannot work.