...
\begin{enumerate}[leftmargin=1.4em,itemsep=1pt,topsep=2pt]
  \item \textbf{An overlap-independent grounding signal (\S\ref{sec:c1}).} A
    mid-layer grounding signal \emph{does} exist beyond lexical overlap: it decodes
    support among answer-present items at AUROC ${\approx}0.87$ (raw; $0.72$--$0.78$
    after purging model confidence) on all four families under item-grouped
    cross-validation. We are explicit about a limit of the design: within the
    answer-present stratum, support and our \emph{synthetic} factuality label are
    identical by construction, so this isolates grounding from \emph{lexical
    overlap} but \emph{not} from factoid-correctness, and we do not claim a grounding
    axis geometrically distinct from a parametric-factuality direction.
  \item \textbf{Cross-family alignment up to a linear map (\S\ref{sec:c3}).} An
    orthogonal-Procrustes map carries the grounding direction between families so the
    ...
    chance ($0.51$); we therefore rest the claim on the cosine. The factuality axis transports identically, indicating a
    shared low-rank contrast rather than a grounding-specific axis; we claim no
  ...
  \item \textbf{Synthetic axes do not transfer to real hallucinations
    ...
    controlled-stimulus grounding direction scores \emph{below} NLI, confidence, and
    ...
    Qwen), so a real grounding signal exists in the internals but is a \emph{different
...
internals-based grounding directions identified on controlled data are real and
remarkably portable across model families, yet are \emph{not}, by themselves,
deployable hallucination detectors---on real data, lexical overlap still dominates.
...
to a grounding direction; Procrustes renders it nearly collinear with the target's
...
\caption{\textbf{Decoupling grounding from lexical overlap.} (a)~The matched
...
\subsection{A grounding axis exists beyond lexical overlap}
\label{sec:c1}
...
$O{=}1$ (cells A vs.\ C), where lexical overlap is held constant. Two caveats bound
this number. First, within $O{=}1$ the support contrast varies \emph{only the
question} (context and answer are held fixed), and our \emph{synthetic} factuality
label is \emph{identical} to support by construction---they diverge only in the
answer-absent cell~D---so supp$_{O=1}$ and a within-stratum factuality probe decode
the same labels and coincide numerically (${\approx}0.877$). This number isolates
grounding from \emph{lexical overlap}, but \emph{not} from factoid-correctness.
Second, in this stratum overlap and answer length are themselves uninformative
(AUROC~$0.500$), so surface removal carries no information here; the operative
confound is \emph{model confidence}. Answer log-prob alone separates $A$ from $C$ at
$0.80$--$0.86$ and NLI at $0.75$; after regressing the hidden states on log-prob and
max-softmax the support contrast drops to $0.70$--$0.78$ (Table~\ref{tab:c1}), which
we take as the primary confound-controlled estimate ($0.87$ is a raw upper bound). A
cross-strata control that fixes the question at training time (train $A$ vs.\ $D$,
evaluate $A$ vs.\ $C$) still transfers at $0.62$--$0.78$---weakest on Qwen and
Gemma---confirming a real but model-dependent grounding component. Because $23.2\%$ of
cell-C items are donor-collision mislabels (\S\ref{sec:construction}), supp$_{O=1}$ is
a \emph{lower} bound: removing them (at the same a-priori layers) lifts it to
$0.99$ ($0.988$--$0.999$). We therefore claim an
overlap-independent, confidence-controlled grounding signal at $0.70$--$0.78$, and we
do \emph{not} claim a grounding axis geometrically distinct from factuality (the
$51$--$67^\circ$ support--factuality angle is a contrast among only three cell-means
and, on matched binary partitions of those means, is not by itself evidence of
distinctness).
...
\toprule
model & $L$ & supp$_{O=1}$ & 95\%\,CI & {\footnotesize$+$conf} & {\footnotesize cross} \\
\midrule
Qwen2.5 & 14 & 0.872 & {\scriptsize[.859,.885]} & 0.70 & 0.62 \\
Mistral & 16 & 0.874 & {\scriptsize[.860,.887]} & 0.78 & 0.78 \\
Llama   & 16 & 0.876 & {\scriptsize[.863,.890]} & 0.72 & 0.78 \\
Gemma   & 21 & 0.878 & {\scriptsize[.864,.892]} & 0.74 & 0.75 \\
\bottomrule
\end{tabular}
\caption{\textbf{C1 (confound-controlled).} Layer fixed a priori at relative depth
$0.5$; GroupKFold on \textsc{item\_id}; polarity resolved on the train fold.
supp$_{O=1}$ is grouped-CV support AUROC within the answer-present stratum (raw upper
bound; item cluster-bootstrap 95\%\,CI). \emph{$+$conf-purge}: after residualizing the
hidden states on confidence (log-prob, max-softmax)---our primary estimate.
\emph{cross-strata}: train $A$ vs.\ $D$ (question fixed), evaluate $A$ vs.\ $C$. Within
$O{=}1$, support and the synthetic factuality label coincide, so we make no
distinctness claim.}
\label{tab:c1}
\end{table}
...
\caption{\textbf{A grounding axis exists beyond overlap, peaking mid-layer.}
...
\subsection{Grounding directions align across families up to a linear map}
\label{sec:c3}
We ask whether the grounding direction occupies the \emph{same} geometry across
...
(transported-to-native cosine ${\approx}0.98$; Table~\ref{tab:c3}), well above anchor
projection. The cosine is far above its null: a random or label-shuffled source
direction transported through the \emph{same} fitted map is essentially orthogonal to
the native direction (cosine $0.01$ and $0.10$ respectively), so $0.976$ is not an
artifact of the shared-stimulus geometry. We deliberately do \emph{not} headline the
transferred-probe AUROC ($0.906$): it equals the within-family bound ($0.905$) and is
therefore ceiling-saturated (a transported probe cannot beat the native one, so AUROC
has no headroom to register loss); scored with polarity-honest (signed) AUROC, random
and shuffled-label directions through the map sit at chance ($0.51$/$0.51$). Critically,
the factuality axis transports \emph{identically} (cosine $0.99$, AUROC $0.915$), so
what aligns is a shared low-rank contrast defined by the matched stimuli, not a
grounding-specific axis. We therefore conclude that grounding
directions \emph{align across these four families up to a linear map} (the cosine
evidence is robust to the nulls), and explicitly avoid claims of near-lossless or
universal transfer.
...
\textsc{Factuality} & 0.990 & 0.915 & 0.911 & --- & --- \\
\bottomrule
\end{tabular}
\caption{\textbf{C3 (cosine-based, leakage-controlled).} Mean over the $12$
cross-family ordered pairs, GroupKFold on \textsc{item\_id} ($3$ seeds), all columns
single-protocol. \emph{cosine}: transported-to-native cosine (the alignment evidence;
non-saturating). A random / label-shuffled source direction transported through the
\emph{same} map has cosine only $0.01$ / $0.10$ with the native direction, so $0.976$
is far above its cosine null. \emph{AUROC}/\emph{within}: transferred-probe vs.\
within-family AUROC---equal, hence ceiling-saturated. \emph{rand-map}/\emph{rand-lab}:
random / shuffled-label source directions through the map, scored with signed
(polarity-honest) AUROC, sit at chance ($0.51$). Factuality aligns identically,
indicating a shared contrast rather than a grounding-specific axis.}
\label{tab:c3}
\end{table}

\begin{figure}[t]\centering
\includegraphics[width=\columnwidth]{figs/fig_c3_heatmap.pdf}
\caption{\textbf{Grounding directions align across families up to a linear map.}
...
\label{sec:c4}
We learn $d_S$ on CtrlPairs and apply it to RAGTruth responses of the same model,
...
real grounding signal therefore exists in the internals, but it is a \emph{different
...
(AUROC). The label-free synthetic grounding axis (synth-$d_S$, support polarity
...
\bottomrule
\end{tabular}
\caption{\textbf{C4, per task type.} Within-task RAGTruth AUROC. The pooled numbers in
Table~\ref{tab:c4} overstate every signal because faithful base rates differ sharply
across tasks (bottom row): overlap's pooled $0.753$ falls to macro $0.647$, NLI's
$0.673$ to $0.582$. A more elaborate sentence-level SummaC-style NLI baseline (max over
context sentences, then mean or min over response sentences) does no better
(macro $0.596$/$0.579$). Within a task, every label-free signal is weak
(${\approx}0.6$); overlap is merely the least-bad.}
...
support is decodable at $0.70$--$0.78$ after purging model confidence, and the
...
factoid-correctness, and even the confidence-purged $0.70$--$0.78$ removes only the
\emph{linear} confidence component, so it remains an upper bound on isolated
grounding; (ii)~cell~B has zero yield, so the fully-orthogonalized
